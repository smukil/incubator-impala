...
int64_t RowBatch::GetDeserializedSize(const RowBatchHeaderPB& header,
  ...
  int64_t result = header.uncompressed_size();
  result += tuple_offsets.size();
  result += header.num_tuples_per_row() * sizeof(int32_t);
  return result;
}

int64_t RowBatch::GetDeserializedSize(const OutboundRowBatch& batch) {
  int64_t result = batch.header_.uncompressed_size();
  result += batch.tuple_offsets_.size();
  result += batch.header_.num_tuples_per_row() * sizeof(int32_t);
  return result;
}

int64_t RowBatch::GetSerializedSize(const OutboundRowBatch& batch) {
  int64_t result = batch.tuple_data_.size();
  result += batch.tuple_offsets_.size();
  result += batch.header_.num_tuples_per_row() * sizeof(int32_t);
  return result;
}
