...
RowBatch::RowBatch(
  ...
  Deserialize(tuple_data, tuple_offsets, input_batch.uncompressed_size,
      input_batch.compression_type);
}
...
RowBatch::RowBatch(const RowDescriptor* row_desc, const ProtoRowBatch& input_batch,
  ...
  Deserialize(input_batch.tuple_data, input_batch.tuple_offsets,
      input_batch.header.uncompressed_size(),
      static_cast<THdfsCompression::type>(input_batch.header.compression_type()));
}

void RowBatch::Deserialize(const Slice& input_tuple_data,
    const Slice& input_tuple_offsets, int64_t uncompressed_size,
  ...
  uint8_t* tuple_data;
  bool is_compressed = compression_type != THdfsCompression::NONE;
  if (is_compressed) {
...
Status RowBatch::Serialize(bool full_dedup, vector<int32_t>* tuple_offsets,
    string* tuple_data, int64_t* uncompressed_size,
    THdfsCompression::type* compression_type) {
