...
class OldDataStreamRecvr::SenderQueue {
 public:
  SenderQueue(OldDataStreamRecvr* parent_recvr, int num_senders, RuntimeProfile* profile);
...
OldDataStreamRecvr::SenderQueue::SenderQueue(OldDataStreamRecvr* parent_recvr, int num_senders,
...
Status OldDataStreamRecvr::SenderQueue::GetBatch(RowBatch** next_batch) {
...
void OldDataStreamRecvr::SenderQueue::AddBatch(const TRowBatch& thrift_batch) {
  unique_lock<mutex> l(lock_);
...
void OldDataStreamRecvr::SenderQueue::DecrementSenders() {
...
void OldDataStreamRecvr::SenderQueue::Cancel() {
  {
    lock_guard<mutex> l(lock_);
    if (is_cancelled_) return;
    is_cancelled_ = true;
...
void OldDataStreamRecvr::SenderQueue::Close() {
  lock_guard<mutex> l(lock_);
...
Status OldDataStreamRecvr::CreateMerger(const TupleRowComparator& less_than) {
  DCHECK(is_merging_);
...
void OldDataStreamRecvr::TransferAllResources(RowBatch* transfer_batch) {
...
OldDataStreamRecvr::OldDataStreamRecvr(OldDataStreamMgr* stream_mgr, MemTracker* parent_tracker,
  ...
  : mgr_(stream_mgr),
    ...
    profile_(profile) {
  mem_tracker_.reset(new MemTracker(-1, "OldDataStreamRecvr", parent_tracker));
  ...
  LOG (INFO) << "Using Thrift OldDataStreamRecvr";
}

Status OldDataStreamRecvr::GetNext(RowBatch* output_batch, bool* eos) {
  DCHECK(merger_.get() != NULL);
  return merger_->GetNext(output_batch, eos);
}

void OldDataStreamRecvr::AddBatch(const TRowBatch& thrift_batch, int sender_id) {
...
void OldDataStreamRecvr::RemoveSender(int sender_id) {
  int use_sender_id = is_merging_ ? sender_id : 0;
  sender_queues_[use_sender_id]->DecrementSenders();
}

void OldDataStreamRecvr::CancelStream() {
  for (int i = 0; i < sender_queues_.size(); ++i) {
    sender_queues_[i]->Cancel();
  }
}

void OldDataStreamRecvr::Close() {
  // Remove this receiver from the OldDataStreamMgr that created it.
  // TODO: log error msg
...
OldDataStreamRecvr::~OldDataStreamRecvr() {
  DCHECK(mgr_ == NULL) << "Must call Close()";
}

Status OldDataStreamRecvr::GetBatch(RowBatch** next_batch) {
  DCHECK(!is_merging_);
