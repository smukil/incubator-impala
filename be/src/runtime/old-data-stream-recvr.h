...
#ifndef IMPALA_RUNTIME_OLD_DATA_STREAM_RECVR_H
#define IMPALA_RUNTIME_OLD_DATA_STREAM_RECVR_H
...
/// OldDataStreamRecvr maintains one or more queues of row batches received by a
/// OldDataStreamMgr from one or more sender fragment instances.
/// Receivers are created via OldDataStreamMgr::CreateRecvr().
/// Ownership of a stream recvr is shared between the OldDataStreamMgr that created it and
/// the caller of OldDataStreamMgr::CreateRecvr() (i.e. the exchange node)
//
...
//
/// OldDataStreamRecvr::Close() must be called by the caller of CreateRecvr() to remove the
/// recvr instance from the tracking structure of its OldDataStreamMgr in all cases.
class OldDataStreamRecvr {
 public:
  ...
  /// Deregister from OldDataStreamMgr instance, which shares ownership of this instance.
  void Close();
 ...
 private:
  friend class OldDataStreamMgr;
  class SenderQueue;

  OldDataStreamRecvr(OldDataStreamMgr* stream_mgr, MemTracker* parent_tracker,
  ...
  /// full. Called from OldDataStreamMgr.
  void AddBatch(const TRowBatch& thrift_batch, int sender_id);

  /// Indicate that a particular sender is done. Delegated to the appropriate
  /// sender queue. Called from OldDataStreamMgr.
  ...
  /// OldDataStreamMgr instance used to create this recvr. (Not owned)
  OldDataStreamMgr* mgr_;
...
#endif /* IMPALA_RUNTIME_OLD_DATA_STREAM_RECVR_H */
