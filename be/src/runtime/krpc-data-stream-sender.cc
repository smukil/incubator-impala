...
class KrpcDataStreamSender::Channel : public CacheLineAligned {
 public:
  ...
  // otherwise.
  Status SerializeAndSendBatch(RowBatch* batch) WARN_UNUSED_RESULT;

  // Send the given serialized row batch to the destination. If the preceding RPC is
  // in progress, this function may block until the previous RPC finishes. Return error
  // status if serialization or the preceding RPC failed. Return OK otherwise.
  Status SendBatch(const ProtoRowBatch* proto_batch);
 ...
 private:
  ...
  // The pointer to the current serialized row batch being sent. It's pointing to the
  // 'proto_batch_' in either entry of 'cached_proto_batches_[]' below.
  ProtoRowBatch* current_proto_batch_ = nullptr;
  ...
  // This is read and written by the main execution thread.
  ...
  ProtoRowBatch* current_proto_batch() { return current_proto_batch_; }
  ...
  // Transmits the serialized row batch 'proto_batch'. This function will wait for
  // any in-flight RPC to complete before calling this function. This is expected to
  // be called only from the fragment instance execution thread. Return error status
  // if initialization of the RPC request parameters failed. Returns OK otherwise.
  Status TransmitData(ProtoRowBatch* proto_batch) WARN_UNUSED_RESULT;
...
void KrpcDataStreamSender::Channel::TransmitDataCompleteCb() {
  ...
  if (LIKELY(controller_status.ok())) {
    ...
    if (LIKELY(status_code == TErrorCode::OK)) {
      DCHECK(current_proto_batch_ != nullptr);
      num_data_bytes_sent_ += RowBatch::GetSerializedSize(*current_proto_batch_);
...
Status KrpcDataStreamSender::Channel::DoTransmitDataRpc() {
  DCHECK(current_proto_batch_ != nullptr);
  DCHECK(current_proto_batch_->IsInitialized());

  rpc_controller_.Reset();
  // Add 'tuple_offsets_' as sidecar.
  int sidecar_idx;
  KUDU_RETURN_IF_ERROR(rpc_controller_.AddOutboundSidecar(
      RpcSidecar::FromSlice(current_proto_batch_->tuple_offsets), &sidecar_idx),
      "Unable to add tuple offsets to sidecar");
  current_proto_batch_->header.set_tuple_offsets_sidecar_idx(sidecar_idx);

  // Add 'tuple_data_' as sidecar.
  KUDU_RETURN_IF_ERROR(rpc_controller_.AddOutboundSidecar(
      RpcSidecar::FromSlice(current_proto_batch_->tuple_data), &sidecar_idx),
      "Unable to add tuple data to sidecar");
  current_proto_batch_->header.set_tuple_data_sidecar_idx(sidecar_idx);

  // Set the RowBatchHeader in the request.
  req_.release_row_batch_header();
  req_.set_allocated_row_batch_header(&current_proto_batch_->header);
...
Status KrpcDataStreamSender::Channel::TransmitData(ProtoRowBatch* proto_batch) {
  std::unique_lock<SpinLock> l(lock_);
  ...
  DCHECK(!rpc_in_flight_);
  DCHECK(current_proto_batch_ != proto_batch);
  rpc_in_flight_ = true;
  current_proto_batch_ = proto_batch;
  RETURN_IF_ERROR(DoTransmitDataRpc());
  return Status::OK();
}

Status KrpcDataStreamSender::Channel::SendBatch(const ProtoRowBatch* proto_batch) {
  CachedProtoRowBatch* next_batch = &cached_proto_batches_[current_batch_idx_];
  *(next_batch->proto_batch()) = *proto_batch;
  VLOG_ROW << "Channel::SendBatch() instance_id=" << fragment_instance_id_
           << " dest_node=" << dest_node_id_
           << " #rows=" << next_batch->proto_batch()->num_rows();
  RETURN_IF_ERROR(TransmitData(next_batch->proto_batch()));
  current_batch_idx_ = (current_batch_idx_ + 1) % num_cached_proto_batches_;
  return Status::OK();
}
...
Status KrpcDataStreamSender::Channel::SerializeAndSendBatch(RowBatch* batch) {
  ...
  VLOG_ROW << "Channel::SerializeAndSendBatch() instance_id=" << fragment_instance_id_
           << " dest_node=" << dest_node_id_
           << " #rows=" << next_batch->proto_batch()->num_rows();
  RETURN_IF_ERROR(TransmitData(next_batch->proto_batch()));
  current_batch_idx_ = (current_batch_idx_ + 1) % num_cached_proto_batches_;
...
Status KrpcDataStreamSender::Send(RuntimeState* state, RowBatch* batch) {
  ...
  if (partition_type_ == TPartitionType::UNPARTITIONED) {
    CachedProtoRowBatch* next_batch = &cached_proto_batches_[current_batch_idx_];
    RETURN_IF_ERROR(SerializeBatch(batch, next_batch));
    ...
    for (int i = 0; i < channels_.size(); ++i) {
      RETURN_IF_ERROR(channels_[i]->SendBatch(next_batch->proto_batch()));
    }
    current_batch_idx_ = (current_batch_idx_ + 1) % num_cached_proto_batches_;
