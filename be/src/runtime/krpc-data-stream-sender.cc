...
class KrpcDataStreamSender::Channel : public CacheLineAligned {
 ...
 private:
  ...
  // into. This is read and written by the main execution thread. It points to 
  ...
  // Called when an RPC call failed. If it turns out that the RPC failed because the
  ...
  // internally by KRPC code (e.g. Connection object was shutdown due to network errors)
  // or if the parent sender has been cancelled or closed since the scheduling of this
  // callback. In which case, MarkDone() will be called with the error status and the
  // RPC is considered complete. 'status' is the error status passed by KRPC code in
  // case the callback was aborted.
  ...
  // It will add 'tuple_offsets_' and 'tuple_data_' in 'current_batch_' as sidecars to
  // the RpcController and store the sidecars' indices to TransmitDataRequestPB sent as
...
void KrpcDataStreamSender::Channel::HandleFailedRPC(const DoRpcFn& rpc_fn,
  ...
  if (RpcMgr::IsServerTooBusy(rpc_controller_)) {
    RpcMgr* rpc_mgr = ExecEnv::GetInstance()->rpc_mgr();
    // RetryCb() is scheduled to be called in a reactor context. This avoids
    // the need to manage yet another thread pool.
