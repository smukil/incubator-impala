...
/// immediately and adds it to 'batch queue' or (2) defer the deserialization and respond
/// to the RPC later if the batch queue is full. In the second case, the batch is added
/// to the receiver's 'deferred_batches_'. When space becomes available in the batch queue,
/// the longest-waiting RPC is removed from the pending sender queue and deserialized.
/// In both cases, the RPC is replied to when the batch has been deserialized and added
/// to the batch queue. The sender will then send its next batch.
///
...
class KrpcDataStreamMgr : public DataStreamMgrBase {
 ...
 private:
  ...
  /// The supported types of tasks for the deserialization thread pools.
  enum DeserializeTaskType {
    EARLY_SENDERS = 0,  // senders which arrive before the receiver is set up
    DEFERRED_BATCHES    // senders deferred due to batch queue being full
  };

  /// A task for the deserialization threads to work on. 'type' specifies the type of the
  /// task to carry out while the other fields identify the receiver of interest.
  struct DeserializeTask {
    /// The type of this task.
    DeserializeTaskType type;
    ...
    /// Sender id. Used only for DEFERRED_BATCHES task.
    int sender_id;
  };
  ...
  /// Enqueue a request to the deserialization thread pool to respond to all the early
  /// senders to the receiver with fragment instance id of 'finst_id' and plan node i
  /// of 'dest_node_id'.
  void TriggerEarlySendersDrain(const TUniqueId& finst_id, PlanNodeId dest_node_id);

  /// Enqueue a request to the deserialization thread pool to drain the deferred batches
  /// for the receiver with fragment instance id of 'finst_id', plan node id of
  /// 'dest_node_id'. 'sender_id' specifies the sender queue if the receiver belongs
  /// to a merging exchange node.
  void TriggerDeferredBatchesDrain(const TUniqueId& finst_id, PlanNodeId dest_node_id,
      int sender_id);
