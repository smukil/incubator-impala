...
#include "runtime/old-data-stream-sender.h"
...
// *Not* thread-safe.
class OldDataStreamSender::Channel : public CacheLineAligned {
 public:
  // Create channel to send data to particular ipaddress/port/query/node
  // combination. buffer_size is specified in bytes and a soft limit on
  // how much tuple data is getting accumulated before being sent; it only applies
  // when data is added via AddRow() and not sent directly via SendBatch().
  Channel(OldDataStreamSender* parent, const RowDescriptor* row_desc,
    ...
    : parent_(parent),
      ...
      rpc_thread_("OldDataStreamSender", "SenderThread", 1, 1,
 ...
 private:
  ...
  Status DoTransmitDataRpc(ImpalaBackendConnection* client,
      const TOldTransmitDataParams& params, TOldTransmitDataResult* res);
};

Status OldDataStreamSender::Channel::Init(RuntimeState* state) {
...
Status OldDataStreamSender::Channel::SendBatch(TRowBatch* batch) {
...
void OldDataStreamSender::Channel::TransmitData(int thread_id, const TRowBatch* batch) {
...
void OldDataStreamSender::Channel::TransmitDataHelper(const TRowBatch* batch) {
  DCHECK(batch != NULL);
  VLOG_ROW << "Channel::TransmitData() instance_id=" << fragment_instance_id_
           << " dest_node=" << dest_node_id_
           << " #rows=" << batch->num_rows;
  TOldTransmitDataParams params;
...
Status OldDataStreamSender::Channel::DoTransmitDataRpc(ImpalaBackendConnection* client,
    const TOldTransmitDataParams& params, TOldTransmitDataResult* res) {
...
void OldDataStreamSender::Channel::WaitForRpc() {
...
Status OldDataStreamSender::Channel::AddRow(TupleRow* row) {
...
Status OldDataStreamSender::Channel::SendCurrentBatch() {
...
Status OldDataStreamSender::Channel::GetSendStatus() {
  WaitForRpc();
  if (!rpc_status_.ok()) {
...
Status OldDataStreamSender::Channel::FlushAndSendEos(RuntimeState* state) {
...
void OldDataStreamSender::Channel::Teardown(RuntimeState* state) {
...
OldDataStreamSender::OldDataStreamSender(int sender_id, const RowDescriptor* row_desc,
...
string OldDataStreamSender::GetName() {
  return Substitute("OldDataStreamSender (dst_id=$0)", dest_node_id_);
}

OldDataStreamSender::~OldDataStreamSender() {
...
Status OldDataStreamSender::Init(const vector<TExpr>& thrift_output_exprs,
    const TDataSink& tsink, RuntimeState* state) {
  DCHECK(tsink.__isset.stream_sink);
  LOG (INFO) << "Using Thrift DataStreamSender";
...
Status OldDataStreamSender::Prepare(RuntimeState* state, MemTracker* parent_mem_tracker) {
...
Status OldDataStreamSender::Open(RuntimeState* state) {
  return ScalarExprEvaluator::Open(partition_expr_evals_, state);
}

Status OldDataStreamSender::Send(RuntimeState* state, RowBatch* batch) {
  DCHECK(!closed_);
  DCHECK(!flushed_);
...
Status OldDataStreamSender::FlushFinal(RuntimeState* state) {
  DCHECK(!flushed_);
  DCHECK(!closed_);
  flushed_ = true;
  for (int i = 0; i < channels_.size(); ++i) {
...
void OldDataStreamSender::Close(RuntimeState* state) {
  if (closed_) return;
  for (int i = 0; i < channels_.size(); ++i) {
...
Status OldDataStreamSender::SerializeBatch(RowBatch* src, TRowBatch* dest, int num_receivers) {
...
int64_t OldDataStreamSender::GetNumDataBytesSent() const {
