...
OldDataStreamMgr::OldDataStreamMgr(MetricGroup* metrics) {
  ...
  LOG (INFO) << "Using Thrift OldDataStreamMgr";
}

inline uint32_t OldDataStreamMgr::GetHashValue(
...
shared_ptr<OldDataStreamRecvr> OldDataStreamMgr::CreateRecvr(RuntimeState* state,
  ...
  shared_ptr<OldDataStreamRecvr> recvr(
      new OldDataStreamRecvr(this, state->instance_mem_tracker(), row_desc,
...
shared_ptr<OldDataStreamRecvr> OldDataStreamMgr::FindRecvrOrWait(
    const TUniqueId& fragment_instance_id, PlanNodeId node_id,
  ...
  *already_unregistered = false;
  {
    ...
    if (closed_stream_cache_.find(promise_key) != closed_stream_cache_.end()) {
      *already_unregistered = true;
      return shared_ptr<OldDataStreamRecvr>();
    }
    shared_ptr<OldDataStreamRecvr> rcvr = FindRecvr(fragment_instance_id, node_id, false);
  ...
  shared_ptr<OldDataStreamRecvr> rcvr =
...
shared_ptr<OldDataStreamRecvr> OldDataStreamMgr::FindRecvr(
  ...
  while (range.first != range.second) {
    shared_ptr<OldDataStreamRecvr> recvr = range.first->second;
...
Status OldDataStreamMgr::AddData(const TUniqueId& fragment_instance_id,
  ...
  bool already_unregistered;
  shared_ptr<OldDataStreamRecvr> recvr = FindRecvrOrWait(fragment_instance_id, dest_node_id,
...
Status OldDataStreamMgr::CloseSender(const TUniqueId& fragment_instance_id,
...
Status OldDataStreamMgr::DeregisterRecvr(
    const TUniqueId& fragment_instance_id, PlanNodeId node_id) {
  ...
  pair<RecvrMap::iterator, RecvrMap::iterator> range =
      receiver_map_.equal_range(hash_value);
  while (range.first != range.second) {
    const shared_ptr<OldDataStreamRecvr>& recvr = range.first->second;
    if (recvr->fragment_instance_id() == fragment_instance_id
        && recvr->dest_node_id() == node_id) {
...
void OldDataStreamMgr::Cancel(const TUniqueId& fragment_instance_id) {
  ...
  while (i != fragment_recvr_set_.end() && i->first == fragment_instance_id) {
    shared_ptr<OldDataStreamRecvr> recvr = FindRecvr(i->first, i->second, false);
    if (recvr.get() == NULL) {
