...
#ifndef IMPALA_RUNTIME_OLD_DATA_STREAM_MGR_H
#define IMPALA_RUNTIME_OLD_DATA_STREAM_MGR_H
...
///   (CloseSender()).
/// - Exchange nodes extract data from an incoming stream via a OldDataStreamRecvr,
///   which is created with CreateRecvr().
//
/// OldDataStreamMgr also allows asynchronous cancellation of streams via Cancel()
/// which unblocks all OldDataStreamRecvr::GetBatch() calls that are made on behalf
...
///
/// TODO: The recv buffers used in OldDataStreamRecvr should count against
/// per-query memory limits.
class OldDataStreamMgr {
 public:
  OldDataStreamMgr(MetricGroup* metrics);
  ...
  /// caller.
  std::shared_ptr<OldDataStreamRecvr> CreateRecvr(RuntimeState* state,
 ...
 private:
  friend class OldDataStreamRecvr;
  ...
  /// CreateRecvr().
  /// we don't want to create a map<pair<TUniqueId, PlanNodeId>, OldDataStreamRecvr*>,
  /// because that requires a bunch of copying of ids for lookup
  typedef boost::unordered_multimap<uint32_t,
      std::shared_ptr<OldDataStreamRecvr>> RecvrMap;
  ...
  /// acquire it.
  std::shared_ptr<OldDataStreamRecvr> FindRecvr(const TUniqueId& fragment_instance_id,
  ...
  std::shared_ptr<OldDataStreamRecvr> FindRecvrOrWait(
  ...
  typedef Promise<std::shared_ptr<OldDataStreamRecvr>> RendezvousPromise;
...
#endif /* IMPALA_RUNTIME_OLD_DATA_STREAM_MGR_H */
