...
shared_ptr<DataStreamRecvrBase> KrpcDataStreamMgr::CreateRecvr(
  ...
  EarlySendersList early_senders;
  {
    ...
    DCHECK(early_senders.waiting_senders.empty());
    DCHECK(early_senders.closed_senders.empty());
  ...
  for (unique_ptr<TransmitDataCtx>& ctx : early_senders.waiting_senders) {
     EnqueueRowBatch({recvr->fragment_instance_id(), move(ctx)});
     num_senders_waiting_->Increment(-1);
  }
  for (unique_ptr<EndDataStreamCtx>& ctx : early_senders.closed_senders) {
...
void KrpcDataStreamMgr::AddData(const TUniqueId& fragment_instance_id,
  ...
  VLOG_ROW << "AddData(): fragment_instance_id=" << fragment_instance_id
           << " node=" << request->dest_node_id() << " size="
  ...
  shared_ptr<KrpcDataStreamRecvr> recvr;
  {
    lock_guard<mutex> l(lock_);
    recvr = FindRecvr(
        fragment_instance_id, request->dest_node_id(), &already_unregistered);
  ...
  if (already_unregistered) {
    ...
    // TODO: respond with a status to indicate to sender that this receiver is closed.
...
void KrpcDataStreamMgr::CloseSender(const TUniqueId& fragment_instance_id,
    const EndDataStreamRequestPB* request, EndDataStreamResponsePB* response,
    RpcContext* context) {
  PlanNodeId dest_node_id = request->dest_node_id();
  ...
  shared_ptr<KrpcDataStreamRecvr> recvr;
  {
    ...
    // It's possible for a sender to issue EOS RPC without sending any rows if no rows
    // are materialized at all in the sender side.
    if (!already_unregistered && recvr == nullptr) {
      RecvrId recvr_id = make_pair(fragment_instance_id, dest_node_id);
...
void KrpcDataStreamMgr::Maintenance() {
  while (true) {
    ...
    for (const EarlySendersList& senders_list : timed_out_senders) {
      for (const auto& ctx : senders_list.waiting_senders) {
        HandleTimedOutSenders<TransmitDataCtx, TransmitDataRequestPB>(ctx);
      }
      for (const auto& ctx : senders_list.closed_senders) {
