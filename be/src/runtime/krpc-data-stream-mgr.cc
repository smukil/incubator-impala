...
shared_ptr<DataStreamRecvrBase> KrpcDataStreamMgr::CreateRecvr(
  ...
  bool has_early_senders;
  {
    ...
    has_early_senders = it != early_senders_map_.end();
  }
  if (has_early_senders) TriggerEarlySendersDrain(finst_id, dest_node_id);
  return recvr;
}
...
void KrpcDataStreamMgr::TriggerEarlySendersDrain(const TUniqueId& finst_id,
    PlanNodeId dest_node_id) {
  DeserializeTask payload =
      {DeserializeTaskType::EARLY_SENDERS, finst_id, dest_node_id, 0};
  deserialize_pool_.Offer(move(payload));
}

void KrpcDataStreamMgr::TriggerDeferredBatchesDrain(const TUniqueId& finst_id,
    PlanNodeId dest_node_id, int sender_id) {
  DeserializeTask payload =
      {DeserializeTaskType::DEFERRED_BATCHES, finst_id, dest_node_id, sender_id};
  deserialize_pool_.Offer(move(payload));
}
...
void KrpcDataStreamMgr::DeserializeThreadFn(int thread_id, const DeserializeTask& task) {
  ...
  if (task.type == DeserializeTaskType::EARLY_SENDERS) {
    EarlySendersList early_senders;
    {
      RecvrId recvr_id = make_pair(task.finst_id, task.dest_node_id);
      lock_guard<mutex> l(lock_);
      EarlySendersMap::iterator it = early_senders_map_.find(recvr_id);
      DCHECK(it != early_senders_map_.end());
    ...
    for (const unique_ptr<TransmitDataCtx>& ctx : early_senders.waiting_sender_ctxs) {
      if (already_unregistered) {
        Status(TErrorCode::DATASTREAM_RECVR_CLOSED, PrintId(task.finst_id),
            task.dest_node_id).ToProto(ctx->response->mutable_status());
        ctx->rpc_context->RespondSuccess();
      } else {
        recvr->AddBatch(ctx->request, ctx->response, ctx->rpc_context);
      }
      num_senders_waiting_->Increment(-1);
    }
  } else {
    DCHECK_EQ(task.type, DeserializeTaskType::DEFERRED_BATCHES);
    if (already_unregistered) return;
    recvr->AddDeferredBatches(task.sender_id);
  }
}
