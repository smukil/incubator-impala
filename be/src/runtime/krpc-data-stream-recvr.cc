...
// KrpcDataStreamRecvr via GetBatch(). If a sender adds a batch that overflows the queue,
// then the sender is added to a queue of 'blocked senders' that will be signalled to try
// again in FIFO order. Senders in that state will not be replied to until their row batch
// is processed, or the receiver is closed; this ensures that only one row-batch per
// sender is buffered in the blocked senders queue.
  ...
  *next_batch = nullptr;
  ...
  while (true) {
    ...
    if (!blocked_senders_.empty()) {
      std::unique_ptr<TransmitDataCtx> ctx = move(blocked_senders_.front());
      l.unlock();
      recvr_->mgr_->EnqueueRowBatch({recvr_->fragment_instance_id(), move(ctx)});
      l.lock();
...
void KrpcDataStreamRecvr::SenderQueue::Close() {
  ...
  batch_queue_.clear();
  // Respond to blocked senders' RPCs.
...
void KrpcDataStreamRecvr::AddBatch(const ProtoRowBatch& proto_batch,
  ...
  sender_queues_[use_sender_id]->AddBatch(proto_batch, request, response, context);
}
