...
class KrpcDataStreamRecvr : public DataStreamRecvrBase {
 ...
 private:
  ...
  /// Add a new row batch to the appropriate sender queue. If the row batch can be
  /// inserted, the RPC will be responded to before this function returns. If the batch
  /// can't be added without exceeding the buffer limit, it is appended to a
  /// 'deferred_batches_' queue for deferred processing. The RPC will be responded to
  /// when the row batch is deserialized later.
  void AddBatch(const TransmitDataRequestPB* request, TransmitDataResponsePB* response,
      kudu::rpc::RpcContext* context);

  /// Drain as many deferred row batches as possible without exceedding the soft limit
  /// of the receiver for the sender queue identified by 'sender_id'. If is_merging_ is
  /// false, it always defaults to queue 0; If is_merging_ is true, the sender queue is
  /// identified by 'sender_id_'.
  void AddDeferredBatches(int sender_id);
