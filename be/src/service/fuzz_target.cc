...
class DataStreamTestFuzz {
 public:
  DataStreamTestFuzz() : next_val_(0) {
  ...
  ~DataStreamTestFuzz() { runtime_state_->ReleaseResources(); }
  ...
  void StartReceiver(TPartitionType::type stream_type, int num_senders, int receiver_num,
    ...
    if (!is_merging) {
      info.thread_handle = new thread(&DataStreamTestFuzz::ReadStream, this, &info);
    } else {
      info.thread_handle = new thread(&DataStreamTestFuzz::ReadStreamMerging, this, &info,
          profile);
    }
  ...
  void StartSender(TPartitionType::type partition_type = TPartitionType::UNPARTITIONED,
    ...
    info.thread_handle =
        new thread(&DataStreamTestFuzz::Sender, this, num_senders, channel_buffer_size,
                   partition_type);
  }
      ...
      for (int k = 0; k < sizeof(receiver_nums) / sizeof(int); ++k) {
        for (int l = 0; l < sizeof(buffer_sizes) / sizeof(int); ++l) {
          for (int m = 0; m < sizeof(merging) / sizeof(bool); ++m) {
            LOG (INFO) << "Running " << i + j + k + l << "th iteration";
            t.TestStream(stream_types[i], sender_nums[j], receiver_nums[k],
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
  ...
  if (SetUpEnv() < 0) return -1;
  impala::RunBasicStreamTest();
  return 0;
}
